...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `Ты эксперт созданием описание хакатона. Прочитав этот текст перепиши описание красивыми словами, с смайликами и т.д. в формате маркдоун. Хакатон будет проходить онлайн в Codigma, поэтому не указывай адрес. Призы в хакатоне не предусмотрены, поэтому не указывай призы. Дату тоже не указывай, так как это будет в другом месте написан. Вот описание который я хочу улучшить:
